...
def generate_graph_instance(n, m, weights, s, u_list, seed, weights_id):
    ...
    X = generate_mixture_sphere_sample(mu, Sigmas, u_list, weights[:m], n)
    G = generate_random_spherical_graph(X)
    return G, {"n": n, "p": p, "m": m, "seed": seed, "weights_id": weights_id, "sigma": s, "noise": False}
...
def generate_dataset_for_n(n, num_graphs=50):
    ...
    for i in tqdm(range(num_graphs)):
        ...
        G, metadata = generate_graph_instance(n, m, weights, s, u_list, seed=7000 * n + i, weights_id=weights_id)
        metadata["dataset_type"] = "vary_all"
